...
export default function PdfToolHub() {
  ...
  const tools = [
    ...
    { id: "comprimir", title: "Comprimir PDF", desc: "Otimização de tamanho", icon: Minimize2, help: "Sabe aquele PDF escaneado que ficou gigantesco? A compressão reduz o peso do arquivo mantendo a qualidade de leitura, ideal para quando você precisa anexar provas e laudos sem estourar a capacidade do sistema." },
    ...
    { id: "converter", title: "PDF p/ Word", desc: "Edição de texto", icon: FileText, help: "Precisa extrair o texto de uma decisão ou contrato em PDF? Esta ferramenta converte a estrutura do PDF para um documento .docx editável. Você economiza horas digitando e pode aproveitar o conteúdo direto no seu editor de texto." },
  ];
  ...
  const handleProcess = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      if (selectedTool.id === "unir") { 
        ...
        pages.forEach((p, i) => download(p, `${baseName}-parte-${i + 1}.pdf`, "application/pdf")); 
      }
        ...
        download(res, `${baseName}-desbloqueado.pdf`, "application/pdf"); 
      }
