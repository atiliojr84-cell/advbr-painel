...
export default function Ticker() {
  ...
  // Criamos uma cópia triplicada para garantir que o loop seja contínuo 
  // e nunca fique vazio na tela enquanto as notícias passam
  const listaExibicao = [...noticias, ...noticias, ...noticias];
  ...
  return (
    <div className="w-full bg-slate-950 text-slate-300 py-3 overflow-hidden border-b border-slate-800 h-12 flex items-center">
      <motion.div 
        ...
        // Inicia em 0 (sem atraso) e vai percorrer um terço da lista duplicada
        initial={{ x: "0%" }}
        animate={{ x: "-33.33%" }}
        transition={{ 
          repeat: Infinity, 
          duration: 180, // Aumentei para 180 segundos para ficar bem devagar (leitura confortável)
          ease: "linear" 
        }}
      >
        {listaExibicao.map((item, i) => (
